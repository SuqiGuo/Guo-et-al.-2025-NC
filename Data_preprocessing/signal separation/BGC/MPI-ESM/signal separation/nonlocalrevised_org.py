...
#DIVCTL_FRC = "/work/bm1147/b380949/web-monitoring/CESM/cover/final_div_yearly/"  #cesm
...
ifile = DIVCTL_FRC + "divbox_cover_fract_yr_lunit_invertlat_1step.nc"   #mpiesm
#ifile = DIVCTL_FRC + "div_ctl_fract_landuses_yearly.nc" #cesm
f = nc.Dataset(ifile, 'r')
DIVCTLFRC = f.variables['const'][:,:,:,:] #mpiesm
#DIVCTLFRC = f.variables['const'][:,:,:,:]  #cesm
...
def write_netcdf( non_local_timeseries, coastline_series, chessboard_extended_series, f, UNIT, LAT_DIFF, NSTEPS,NSTEP, NLAT, NLON):
    ...
    f.createVariable( VAR + "_nonlocal", np.float64,('time','landuse','lat','lon'), fill_value=FILL_VALUE)
    ...
    f.createVariable( "coastline", np.float64,('landuse','lat','lon'), fill_value=FILL_VALUE)
        #suq2020 add one dim 'lev'
    ...
    f.createVariable( "chessboard_extended", np.float64,('landuse','lat','lon'), fill_value=FILL_VALUE)
        #suq2020 add one dim 'lev'
