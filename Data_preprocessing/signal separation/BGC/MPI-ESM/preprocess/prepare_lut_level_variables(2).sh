...
CMOR_VAR_LIST="fracLut" # cesm model output data used with cesm table
...
aim_dir=/work/bm1147/b380949/web-monitoring/cover
for CMOR_VAR in ${CMOR_VAR_LIST}; do
for exp in ${exp_id}; do
    if [ "$exp" == "CROP" ]; then
        destination=${aim_dir}/crop
    elif [ "$exp" == "FRST" ]; then
        destination=${aim_dir}/frst
    elif [ "$exp" == "IRRI" ]; then
        destination=${aim_dir}/irri
    elif [ "$exp" == "HARV" ]; then
        destination=${aim_dir}/harv
    fi
if [ "$exp" == "CROP" ]; then
    DIR=${root_dir}/${exp}/${sub_dir}/${CMOR_TABLE}/backup/$CMOR_VAR/gn/v20190710/
else
